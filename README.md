...
### 📂 로그 파싱 & 필터링
- Serilog 포맷 자동 파싱 (타임스탬프, 레벨, 유저ID, 핸들러 추출)
- 로그 레벨 / 핸들러 / 유저 / 텍스트 필터
- 에러/경고 전용 보기
- 드래그 & 드롭, 클립보드 붙여넣기 지원
...
### 🔄 로컬 파일 감시
- FileSystemWatcher로 로그 파일 변경 실시간 감지
- 새로 추가된 라인 자동 표시
...
[📥 Releases](https://github.com/lee775/LogAnalyzer-releases/releases)에서 최신 `LogAnalyzer.exe`를 다운로드하세요.

- .NET 런타임 불필요 (self-contained)
- Windows x64 전용
- 설치 없이 exe 실행만으로 사용 가능
